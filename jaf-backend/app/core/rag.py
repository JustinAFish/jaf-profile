...
class RAGPipeline:
    ...
    ) -> Union[AssistantResponse, Tuple[str, List[Source]]]:
        """
        Run retrieval → rerank → threshold gate → prompt building. Returns the canned
        escalation AssistantResponse when no document clears the threshold (short-circuits
        before any LLM call), otherwise (prompt_content, display_sources).
        """
        threshold = (
            relevance_threshold
    ...
    async def get_response(
        ...
        try:
            prepared = await self._prepare(
                question, conversation_history, relevance_threshold
            )
            if isinstance(prepared, AssistantResponse):
                return prepared

            prompt_content, sources = prepared
    ...
    async def stream_response(
        ...
        conversation_history: Optional[List[MessageHistory]] = None,
        relevance_threshold: Optional[float] = None,
    ) -> AsyncIterator[Union[str, AssistantResponse]]:
        """
        Streaming variant of get_response: yields answer text deltas (str), then a final
        AssistantResponse whose answer is empty — sources/escalation ride on the final item.
        Escalation and pipeline errors yield their canned answer as a single text delta.
        """
        try:
            prepared = await self._prepare(
                question, conversation_history, relevance_threshold
            ...
            if isinstance(prepared, AssistantResponse):
                ...
                yield AssistantResponse(
                    ...
                    needs_escalation=prepared.needs_escalation,
                    escalation_reason=prepared.escalation_reason,
                )
                return

            prompt_content, sources = prepared
            total = 0
            async for delta in self.llm_service.stream_chat_response(prompt_content):
                total += len(delta)
                yield delta
            logger.info("LLM streamed response length: %s", total)
            ...
            logger.exception("Error in streaming RAG pipeline")
            yield (
                "I apologize, but I couldn't find any sufficiently relevant "
                "information in our documentation. Please contact Justin directly "
                "to support you with this question. He will be happy to help you."
